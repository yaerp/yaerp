...
def journal_entries_table(journal_entries_iterable):

    # Table data which demonstrates handling of wrapping and text styles
    data_list: List[List[Any]] = list()
    data_list.append(["J/E no. 16", "", "", ""])
    data_list.append(["2023-02-03", "Sale", "1000", "",])
    data_list.append(["", "    Cash", "", "900",])
    data_list.append(["", "    Tax", "", "100",])
    data_list.append(["", blue("Sale office equipment: 2 komputers, 1 printer and shredder"), "", "",])

    # Table Columns (width does not account for any borders or padding which may be added)
    columns: List[Column] = list()
    columns.append(Column("", width=12))
    columns.append(Column("Account", width=38))
    columns.append(
        Column("Debit", width=12, header_horiz_align=HorizontalAlignment.CENTER, data_horiz_align=HorizontalAlignment.RIGHT)
    )
    columns.append(
        Column("Credit", width=12, header_horiz_align=HorizontalAlignment.CENTER, data_horiz_align=HorizontalAlignment.RIGHT)
    )
    st = SimpleTable(columns)
    table = st.generate_table(data_list)
    ansi_print(table)
